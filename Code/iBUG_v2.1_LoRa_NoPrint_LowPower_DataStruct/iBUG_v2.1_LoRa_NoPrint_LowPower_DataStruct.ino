...
// Read BME688
  ...
  uint16_t temp = bme.temperature*100;
  uint16_t hm = bme.humidity*100;
  uint16_t pr = bme.pressure*100;
  uint16_t gs= bme.gas_resistance*100; 


  m_lora_app_data.port = gAppPort;
  m_lora_app_data.buffer[i++] = 0x01; //address
  m_lora_app_data.buffer[i++] = (uint8_t)(temp >> 8);  // temperature 
  m_lora_app_data.buffer[i++] = (uint8_t)temp;         // temperature
  m_lora_app_data.buffer[i++] = (uint8_t)(hm >> 8);  // humidity
  m_lora_app_data.buffer[i++] = (uint8_t)hm;         // humidity
  m_lora_app_data.buffer[i++] = (uint8_t)((pr & 0xFF000000) >> 24); //Pressure
  m_lora_app_data.buffer[i++] = (uint8_t)((pr & 0x00FF0000) >> 16);
  m_lora_app_data.buffer[i++] = (uint8_t)((pr & 0x0000FF00) >> 8);
  m_lora_app_data.buffer[i++] = (uint8_t)(pr & 0x000000FF);
  m_lora_app_data.buffer[i++] = (uint8_t)((gs & 0xFF000000) >> 24);//gas resistance
  m_lora_app_data.buffer[i++] = (uint8_t)((gs & 0x00FF0000) >> 16);
  m_lora_app_data.buffer[i++] = (uint8_t)((gs & 0x0000FF00) >> 8);
  m_lora_app_data.buffer[i++] = (uint8_t)(gs & 0x000000FF);

 
  m_lora_app_data.buffsize = i;
  Serial.printf("Data %d\n", m_lora_app_data);
  data = "Tem:" + String(temp) + "C " + "Hum:" + String(hm) + "% " + "Pres:" + String(pr) + "KPa " + "Gas:" + String(gs) + "Ohms";
  Serial.println(data);
